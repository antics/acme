...
textbswidth(Text *t, Rune c)
{
	...
	if(c == 0x08 || c == 0x04)	/* ^H: erase character or ^D */
		return 1;
	q = t->q0;
	skipping = TRUE;
	while(q > 0){
...
texttype(Text *t, Rune r)
{
	...
	*/
	...
	case 0x06: // C-f
		typecommit(t);
		//
		// NOTE: file-> is buffer (the data), b->nc is end of data
		//
		if(t->q1 < t->file->b.nc)
 ...
 *	from: https://github.com/karahobny/acme2k/blob/master/src/cmd/acme/text.c
 */
	...
	//
	// TODO:
	// ^D should eat characters and move the line one step left
	// Now it's the same as ^H
	//
	// ^K should cut line
	//
	case 0x04: /* ^D */
